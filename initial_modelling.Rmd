---
title: "GLMs for Car Insurance Pricing: Initial GLM Modelling"
...
`policyaggclaim_tbl` table but keep the other two tables in the workspace
as they may be useful later.

```{r load_data, echo=TRUE}
policy_tbl         <- read_feather("data/policy_data.feather")
claim_tbl          <- read_feather("data/claim_data.feather")
policyaggclaim_tbl <- read_feather("data/policyaggclaim_data.feather")
```
...
claimrate_region_tbl <- policyaggclaim_tbl %>%
    group_by(region) %>%
...
claimrate_driverage_tbl <- policyaggclaim_tbl %>%
...
model_fuel <- glm(claim_count ~ 0 + gas
                 ,offset = log(exposure)
                 ,family = poisson
                 ,data   = policyaggclaim_tbl)
...
`gas` to be added to the intercept - we want two seperate values shown
in the output.
...
test_tbl <- data_frame(gas     = c("Diesel", "Regular")
                      ,exposure = 1)

predict(model_fuel, newdata = test_tbl, type = 'response')
```
...
model1 <- glm(claim_count ~ cat_driver_age + density + region
             ,offset = log(exposure)
             ,family = poisson
             ,data   = policyaggclaim_tbl)
...
model2 <- glm(claim_count ~ cat_driver_age + region
             ,offset = log(exposure)
             ,family = poisson
             ,data   = policyaggclaim_tbl)
...
model3 <- glm(claim_count ~ cat_driver_age + region + cat_car_age + power +
                            brand + gas
             ,offset = log(exposure)
             ,family = poisson
             ,data   = policyaggclaim_tbl)
...
glimpse(claimdata_tbl)

summary(claimdata_tbl)
```
...
model6 <- lm(log(claim_amount) ~ brand + cat_car_age + cat_driver_age + gas +
                                 region
            ,data = claimdata_tbl)
...
model7 <- glm(claim_amount ~ brand + cat_car_age + cat_driver_age + gas +
                             region
             ,family = Gamma(link = 'log')
             ,data   = claimdata_tbl)
...
claim_15000_tbl <- claimdata_tbl %>%
    filter(claim_amount < 15000)

model10 <- lm(log(claim_amount) ~ cat_car_age + gas
             ,data = claim_15000_tbl)

summary(model10)

ggplot(claim_15000_tbl) +
    geom_line(aes(x = claim_amount, y = claim_amount), size = 0.25) +
...
model11 <- glm(claim_amount ~ cat_car_age + gas
              ,family = Gamma(link = 'log')
              ,data   = claim_15000_tbl)
...
ggplot(claim_5000_tbl) +
    geom_density(aes(x = claim_amount)) +
    facet_grid(cat_car_age ~ gas, scales = 'free') +
...
```{r claim_amount_boruta, echo=TRUE, cache=TRUE}
claimdata_boruta_tbl <- claimdata_tbl %>%
    select(-c(policy_id, exposure))

claimdata_boruta <- Boruta(claim_amount ~ .
                          ,data    = claimdata_boruta_tbl
                          ,doTrace = 2
                          ,ntree   = 100)
...
We also want to look at the plotted output.

```{r claim_amount_boruta_plot, echo=TRUE}
claimdata_boruta %>% plot
```


We will try fitting one last Gamma distribution using the features
from Boruta: `car_age`, `driver_age`, `brand`, `gas`, `region`, `density`.

```{r claim_boruta_glm, echo=TRUE, cache=TRUE}
model12 <- glm(claim_amount ~ car_age + driver_age + brand + gas + region +
                              density
              ,family = Gamma(link = 'log')
              ,data   = claim_5000_tbl)
